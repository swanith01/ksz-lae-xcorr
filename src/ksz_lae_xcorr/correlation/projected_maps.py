...
def build_projected_maps(cfg, field_data: dict, tracer_data: dict, seeds: list[int]) -> dict:
    ...
    tracer_data[seed] = {'z_nodes', <tracer>_count_lc for whichever tracers were stitched}

    Returns a dict of dicts, one entry per map name, each keyed by seed:
    {'kSZ','kSZ2','xe2','v2','v_proj','v_proj2', <one entry per tracer in cfg.correlation.tracers
    that was actually stitched>}. Tracers not yet available (e.g. LAE/LBG before the external
    catalogues are handed over) are simply omitted from the output rather than filled with
    zeros -- callers should check `seed in maps[tracer_name]` rather than assuming all
    configured tracers are always present.
    """
    ...
    tracer_names = list(cfg.correlation.tracers)
    count_key = {"halo": "halo_count_lc", "lae": "lae_count_lc", "lbg": "lbg_count_lc"}

    out = {k: {} for k in ("kSZ", "kSZ2", "xe2", "v2", "v_proj", "v_proj2")}
    out.update({t: {} for t in tracer_names})
    ...
    for seed in seeds:
        ...
        for tracer in tracer_names:
            key = count_key[tracer]
            if key not in tracer_data[seed]:
                continue  # this tracer wasn't stitched (e.g. LAE/LBG pending handover)
            proj = tracer_data[seed][key][:, :, zi_lo_t:zi_hi_t].sum(axis=2)
            out[tracer][seed] = make_overdensity(proj).astype(np.float32)
